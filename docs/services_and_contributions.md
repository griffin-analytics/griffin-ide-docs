...
In this section we describe how Griffin Analytics Studio extensions can use services provided by the platform and by other extensions. Furthermore, we describe how extensions can contribute to the Griffin Analytics Studio workbench via contribution points.

A **service** is an object that provides functionality to its consumers. The contract between a service and its consumers is described by an interface. Any implementation of a service must implement that interface according to the interface documentation. Any extension in Griffin Analytics Studio can provide and/or consume services. The extensions provided by the Griffin Analytics Studio platform provide a set of default services, e.g. the `MessageService`. However, you can provide and consume your own custom services, too.

**Contribution points** define hooks, which allow extending something. Contribution points are defined by an interface that the contributor is expected to implement, e.g. a `CommandContribution`. The extension defining the contribution point will then pick up the contribution, e.g. adding the contributed command to the Griffin Analytics Studio workbench.

Contribution points, like services, can be contributed to and defined by any extension. The Griffin Analytics Studio platform defines a set of default contribution points, e.g. to add commands or menus to the Griffin Analytics Studio workbench. However, you can also define your own ones.

The usage of services and contribution points requires communication between extensions. To avoid direct dependencies on the implementation classes, Griffin Analytics Studio uses dependency injection.
...
Griffin Analytics Studio uses the InversifyJS DI framework to wire up the different services and contribution points.
...
Dependency injection is an integral part of Griffin Analytics Studio. Therefore, we highly recommend learning at
least the basics of InversifyJS. For more details, please also refer to articles on how dependency injection works in Griffin Analytics Studio.

## Using Services

To use a service in Griffin Analytics Studio, you can get it injected as a dependency using DI. Dependencies are usually specified via the interface of the service you want to retrieve. This way, you even avoid a dependency on any specific implementation, the caller just knows the interface. This allows the component providing the implementation to seamlessly replace the implementation of a service. You can even override the existing default implementation of a service without breaking any service consumers.
...
Contribution Points in Griffin Analytics Studio define an interface to be implemented, e.g. `CommandContribution`. A contributing extension must provide an implementation of this interface and mark it with `@injectable`, e.g.:
...
Please see Commands/Menus/Keybindings for a simple example for the usage of services and contribution points.
...
registering `OpenHandler`s. You can have a look at the code for examples.

Griffin Analytics Studio comes with an extensive list of contribution points already. A good way
