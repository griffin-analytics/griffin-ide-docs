...
Griffin Analytics Studio users can execute tasks to automate certain steps in their workflow.
...
Tasks in Griffin Analytics Studio are structurally compatible with Visual Studio Code tasks.
Similar to Visual Studio Code, users can define tasks in a file named `tasks.json` file in the workspace or on user level.

## TaskProviders, TaskResolvers and TaskRunners

In Griffin Analytics Studio, tasks can also be provided from and executed by custom Griffin Analytics Studio extensions.
In particular, Griffin Analytics Studio offers the three main contribution points for tasks: `TaskProvider`, `TaskResolver` and `TaskRunner`.
To better understand the purpose of those, let’s look at the flow, through the involved components, when a user selects and executes a task.

Besides the user-defined task configurations, Griffin Analytics Studio will also offer its users all task configurations collected from the registered task providers.
...
Griffin Analytics Studio provides dedicated contribution points for task providers, task resolvers, and task runners.
Thus, Griffin Analytics Studio extensions can extend the list of available tasks with custom task types, handle the resolution of task configurations of custom task types, as well as implement their execution.
...
Our `TaskContribution` contributes a task provider and a task resolver (see following listing). Their implementation is shown below. By specifying a task type along with the registration, Griffin Analytics Studio will pick the right resolver and runner for our custom task type (`myTaskType`).
...
Using the existing interface `Task` allows us to connect the task to the `TaskManager` which will show visual feedback in the workbench during tasks execution.
...
Our custom task will run for 5000 ms and then stop.
...
To support users in the creation of such user-defined task configurations for custom task types, Griffin Analytics Studio provides a dedicated contribution point for *task definitions*.
