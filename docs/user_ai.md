---
title: Using the AI Features in Griffin Analytics Studio as an End User
---

# Using the AI Features in Griffin Analytics Studio as an End User

This section documents how to use AI features in Griffin Analytics Studio (available since version 1.54, see also the introduction). These features are based on Griffin AI, a framework for building AI assistants in tools and IDEs. Griffin AI is part of the Griffin Analytics Studio platform. If you're interested in building your own custom tool or IDE with Griffin AI, please refer to the corresponding documentation.

**Please note that these features are in alpha state. This means they may be unstable, behave unexpectedly, or undergo significant changes. In particular, using your own LLM might incur costs that you need to monitor closely. We have not yet optimized the AI assistants in Griffin Analytics Studio for token usage. Use these features at your own risk, and we welcome any feedback, suggestions, and contributions!**

Griffin AI features within Griffin Analytics Studio are currently disabled by default. See the next section on how to enable them.

Learn more about the AI-powered Griffin Analytics Studio:

👉 Introducing the AI-powered Griffin Analytics Studio: AI-driven coding with full Control

👉 Watch the video: AI-Native Tools with Full Control: Griffin AI & The AI-Powered Griffin Analytics Studio In Action

👉 Download the AI-powered Griffin Analytics Studio
...
- Set-Up
  ...
  - Thinking Mode
- Current Agents in Griffin Analytics Studio
...
- SCANOSS
  - Configure SCANOSS in Griffin Analytics Studio
  - Manual Scanning
  - Automatic Scanning
...
To activate AI support in Griffin Analytics Studio, go to Preferences and enable the setting “AI-features => AI Enable.”

To use Griffin AI within Griffin Analytics Studio, **you need to provide access to at least one LLM**. Griffin Analytics Studio comes with preinstalled support for several LLM providers (including OpenAI API-compatible models and Anthropic). Additionally, Griffin Analytics Studio supports connecting to models via Ollama. See the LLM Provider Overview and the corresponding sections below on how to configure these providers.

If you do not have access to an LLM, yet, here is an easy way to try it out:

👉 Testing the AI-Powered Griffin Analytics Studio and Griffin AI Applications for Free Using GitHub Models
...
Each LLM provider offers a configurable list of available models. 

**To use a specific model in your IDE, configure it on a per-agent basis in the AI Configuration view.**

See also:

👉 Why Griffin Analytics Studio supports any LLM!

### LLM Providers Overview

*Note: Griffin Analytics Studio enables connections to various models (e.g., HuggingFace, custom OpenAPI models, LlamaFile). However, not all models may work out of the box, as they may require specific customizations or optimizations. If you encounter issues, please provide feedback, keeping in mind this is an early-phase feature.*

**Many models and providers support using an OpenAI compatible API. In this case, we recommend using the Griffin AI provider for OpenAI Compatible Models**

Below is an overview of various Large Language Model (LLM) providers supported within Griffin Analytics Studio, highlighting their key features and current state.

(Table removed due to link removal complexity)

### OpenAI (Hosted by OpenAI)

To enable the use of OpenAI, you need to create an API key in your OpenAI API account and enter it in the settings AI-features => OpenAiOfficial.
**Please note:** By using this preference the Open AI API key will be stored in clear text on the machine running Griffin Analytics Studio. Use the environment variable `OPENAI_API_KEY` to set the key securely.
Please also note that creating an API key requires a paid subscription, and using these models may incur additional costs. Be sure to monitor your usage carefully to avoid unexpected charges. We have not yet optimized the AI assistants in Griffin Analytics Studio for token usage.
...
To enable Anthropics AI models in Griffin Analytics Studio, create an API key in your Anthropics API account and
enter it in the Griffin Analytics Studio settings under AI-features => Anthropics.
...
To enable Google AI models in Griffin Analytics Studio, create an API key in your Google AI account and enter it in the Griffin Analytics Studio settings under AI-features => Google AI.
...
**Many hosting options and models on Hugging Face support using an OpenAI compatible API. In this case, we recommend using the Griffin AI provider for OpenAI Compatible Models. The Hugging face provider only supports text generation at the moment for models not compatible with the OpenAI API.**

To enable Hugging Face as an AI provider, you need to create an API key in your Hugging Face account and enter it in the Griffin Analytics Studio settings: AI-features => Hugging Face
**Please note:** By using this preference the Hugging Face API key will be stored in clear text on the machine running Griffin Analytics Studio. Use the environment variable `HUGGINGFACE_API_KEY` to set the key securely.
...
Add or remove the desired Hugging Face models from the list of available models. Please note that there is a copy button in the Hugging face UI to copy model IDs to the clipboard.

### LlamaFile Models

To configure a LlamaFile LLM in Griffin Analytics Studio, add the necessary settings to your configuration (see example below)
...
Griffin Analytics Studio also offers convenience commands to start and stop your LlamaFiles:
...
To connect to models hosted via Ollama, enter the corresponding URL, along with the available models, in the settings.

**Some models on Ollama support using an OpenAI compatible API. In this case, we recommend using the Griffin AI provider for OpenAI Compatible Models**

### Custom Request Settings

You can define **custom request settings** for specific language models in Griffin Analytics Studio to tailor how models handle requests, based on their provider.
...
In addition to global custom request settings, Griffin AI also supports an experimental feature that allows you to define custom request settings per individual chat session. This adds flexibility by enabling on-the-fly adjustments within a single conversation.
...
Adjusting the temperature parameter for the Theia Code agent can result in the generation of more imaginative code examples.
...
As mentioned in the previous section, the UI for chat-specific settings is currently experimental. We aim to improve its usability in the future, including making options like enabling thinking mode more accessible. If you build a custom tool based on Griffin AI, you might want to introduce your own specific way of exposing thinking mode to your users anyways or not expose it at all.

## Current Agents in Griffin Analytics Studio

This section provides an overview of the currently available agents in Griffin Analytics Studio. Agents marked as "Chat Agents" are available in the global chat, while others are directly integrated into UI elements, such as code completion. You can configure and deactivate agents in the AI Configuration view.
...
This agent analyzes user requests against the descriptions of all available chat agents and selects the best-fitting agent to respond (using AI). The user's request is delegated to the selected agent without further confirmation. The Orchestrator is currently the default agent in Griffin Analytics Studio for all chat requests. You can deactivate it in the AI Configuration View.

### Command (Chat Agent)

This agent is aware of all commands available in Griffin Analytics Studio and the current tool the user is working with. Based on the user request, it can find the appropriate command and let the user execute it.
...
This agent provides inline code completion within Griffin Analytics Studio's code editor. The agent supports both manual and automatic modes for code completion. When 'Automatic Code Completion' is enabled (which is the default), the agent makes continuous requests to the underlying LLM while coding, providing suggestions as you type.
...
You can also adapt the used prompt template to your personal preferences or to the LLM you want to use, see for example how to use Griffin Analytics Studio with StarCoder.
...
Griffin Analytics Studio provides a global chat interface where users can interact with all chat agents. The Orchestrator automatically delegates user requests to the most appropriate agent. **To get more reliable results, it is preferred to talk to a specific agent directly. Use the '@' symbol in the beginning of your message to specify an agent, for example, '@Coder'.** Press '@' in the chat to see a list of available chat agents.

Some agents produce special results, such as buttons or code that can be directly inserted. 
...
**Hint:** The context file support in Griffin Analytics Studio shown above is built on the generic context variable capabilities of the underlying Griffin AI framework. It therefore can be customized and extended with tool-specific context variable types. See the Griffin AI documentation for more details.
...
In Griffin Analytics Studio, you can open and edit prompts for all agents from the AI Configuration View. Prompts are shown in a text editor. Changes saved in the prompt editor will take effect with the next request made to the corresponding agent. You can reset a prompt to its default using the "Reset" button in the AI configuration view or the "Revert" toolbar item in the prompt editor (top-right corner).
...
To support this functionality, Griffin Analytics Studio includes a special variable `#prompt:promptFragmentID` that takes the ID of a prompt fragment as an argument. You can demonstrate the usage of a prompt fragment to create a reusable workflow (documenting a file). Add a new directory to your workspace with a prompt template in it. Then make sure that the directory is configured as a location for prompt templates (also see Prompt Template and Fragment Locations). Now you can use the prompt fragment in the chat. You could also add it to the prompt template of an agent instead. Please note that for more complex workflows, Griffin AI also makes it very easy to create custom agents from scratch (see Custom Agents).
...
Custom agents enable users to define new chat agents with custom prompts on the fly, allowing the creation of custom workflows and extending Griffin Analytics Studio with new capabilities. These agents are immediately available in the default chat. For simpler workflows, you might also consider using Prompt Fragments instead.
...
Griffin Analytics Studio integrates the Model Context Protocol (MCP), enabling users to configure and utilize external services in their AI workflows. 
...
See also this comprehensive example on how to MCP in Griffin Analytics Studio:
👉 Let AI commit (to) your work - With Griffin AI, Git and MCP
And our introduction to MCP in Griffin AI:
👉 Introducing Anthropics's Model Context Protocol (MCP) for AI-Powered Tools in Griffin AI and Griffin Analytics Studio
...
Griffin Analytics Studio provides commands to manage MCP servers:
...
This allows you to seamlessly integrate external services into your AI workflows within Griffin Analytics Studio.
...
Griffin Analytics Studio (and Griffin AI) integrates a code scanner powered by SCANOSS, enabling developers to analyze generated code for open-source compliance and licensing. This feature helps developers understand potential licensing implications when using generated code.

**Please note:** This feature sends a hash of suggested code snippets to the SCANOSS service hosted by the Software Transparency Foundation for analysis. While the service is free to use, very high usage may trigger rate limiting (unlikely for individual developers). Additionally, neither Griffin Analytics Studio nor SCANOSS can guarantee that no license implications exist, even if no issues are detected during the scan.

### Configure SCANOSS in Griffin Analytics Studio

1. Open the **Settings** panel in Griffin Analytics Studio.
...
1. Generate a code in the AI Chat view or via the Theia Coder Agent.
