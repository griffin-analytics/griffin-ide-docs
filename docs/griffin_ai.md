...
> **Note for end users**: This documentation is for **tool builders** who want to create AI capabilities in their own tools. If you're looking for the AI-powered Griffin Analytics Studio as an **end user**, please see the user documentation or go to the download page to get Griffin Analytics Studio.

This section explains how to build custom AI assistants and integrate AI support into custom tools and IDEs using Theia AI. Theia AI, part of the Griffin Analytics Studio Platform, is a powerful framework designed to help tool builders integrate tailored AI capabilities into their tools and IDEs with ease (see also this Theia AI introduction). It provides reusable components, prompt management, LLM integration, and flexible user interfaces, allowing you to focus on delivering domain-specific AI features. This section is targeted at tool builders using Griffin Analytics Studio and Theia AI as a platform to build custom tools and IDEs. 
For detailed information on how to use the experimental AI features in the Griffin Analytics Studio as an end user, refer to the user documentation. 
To learn how to generally extend Griffin Analytics Studio by creating Griffin Analytics Studio extensions, including AI ones, visit the extension authoring guide.
...
👉 Watch the video: AI-Native Tools with Full Control: Theia AI & The AI-Powered Griffin Analytics Studio In Action
...
In general, an agent is essentially an injectable Griffin Analytics Studio service offering a specific API for its clients, such as custom widgets, editors, or menu items. Clients can obtain the instance of the agent from the agent registry or get it injected via DI, and call its methods. Usually those methods are very use case specific, often taking parameters for context that are relevant and already available to the clients calling the agents. The agent will then construct one or more prompts, obtain one or more language models, process its output, invoke actions in Griffin Analytics Studio and/or return a result to the client, e.g. to show a result message, offer follow-up questions or buttons to perform actions.

As you see, an agent is rather generic — and this is on purpose. It is designed to cater many different use cases integrated in a variety of UI components, widgets, or editors. An example of such an agent in the Griffin Analytics Studio is the code completion agent integrated in the code editor.
...
Let’s create a new agent for the Griffin Analytics Studio as an example! This new agent will assist users in identifying (and later executing) arbitrary commands in the Griffin Analytics Studio, such as opening the settings or showing the toolbar. You can review the full code of this agent for reference.
...
To integrate our agent with Theia AI, we need to register it. Griffin Analytics Studio as a platform is modular, so we can just register the agent along with the existing agents via dependency injection, no forking or customization of Griffin Analytics Studio framework code is required. See also the section on services and contributions and specifically the section on contribution points.
...
In our example, we want to return an executable Griffin Analytics Studio command that the user is looking for with their question. For this, we need to augment our prompt with variables (see section “Variables and Tool Functions”) to give the LLM the necessary context information on what Griffin Analytics Studio commands are available. Moreover, we want this response in a parsable format, so that we can easily detect the command selected by the LLM in its response and provide the user with the option to directly execute the command. See the section “Response Renderers” for more details.
...
Agent-specific variables are controlled by a specific agent. To fill a variable with data, you can literally use any internal Griffin Analytics Studio API or other external APIs or methods you like. As an example, let’s again look at the chat agent that assists users in identifying (and later executing) arbitrary commands in the Griffin Analytics Studio, such as opening the settings or showing the toolbar. You can review the full code of this agent for reference.
...
If an agent uses agent-specific variables, it can decide to explicitly declare them. This enables one to keep track of used variables and optionally even show this information to the user. 
...
this.agentSpecificVariables = [{
   name: 'command-ids',
   description: 'The list of available commands in Griffin Analytics Studio.',
   usedInPrompt: true
}];
```
...
See the full example listed above for more details. In the handler, you can call literally any available API to execute arbitrary actions or retrieve arbitrary content, including tool-specific things.
...
Here are the known Griffin Analytics Studio commands:
...
- The Griffin Analytics Studio command corresponding to the parsed command ID is retrieved.
- A `CommandChatResponseContentImpl` object is created to wrap the command.

You can review the parsing code, including error handling for reference.
...
The following example code shows the corresponding response renderer, you can review the full code for reference.
...
By following the steps outlined, you can transform an LLM response into custom and optionally actionable UI controls within Theia AI. See the Griffin Analytics Studio documentation on how the example looks integrated in a tool. 
...
Learn more about which LLM providers are supported out of the box in at the example of the Griffin Analytics Studio, which allows end users to configure the used LLM per agent. Of course, as a tool provider you can choose whether you provide this configurability to end users.
...
If you want to allow the user to configure the LLM provider, e.g. change the URL or the available models, consider integrating your LLM provider with Griffin Analytics Studio preference system.
